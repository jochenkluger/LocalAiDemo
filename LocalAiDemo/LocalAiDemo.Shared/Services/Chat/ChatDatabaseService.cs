 ...
 Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), Constants.AppFolder, dbName);
#else
            var databasePath =
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), Constants.AppFolder,
                    dbName);
#endif
        ...
        public async Task InitializeDatabaseAsync()
        {
            try
            {
                ...
                try
                {
                    // Try to create the vector index table for chats - if this fails, vector search
                    // is not available
                    using (var command = _databaseConnection.CreateCommand())
                    {
                        command.CommandText = @"
            ...
            try
            {
                ...
                if (contacts.Count < 5) // Ensure we have at least 5 contacts
                {
                    ...
                    if (chatCount == 0 && contacts.Count > 0)
                    {
                        ...
                        var sampleChats = new List<Models.Chat>
                        {
        ...
        public async Task<List<Models.Chat>> GetChatsByContactAsync(int contactId)
        {
            try
            {
                var chats = new List<Models.Chat>();
        ...
        public async Task<Models.Chat?> GetOrCreateChatForContactAsync(int contactId)
        {
            try
            {
        ...
        private async Task<int> SaveChatMessageAsync(ChatMessage message, int chatId)
        {
            try
            {
                ...
                if (_vectorSearchAvailable && message.EmbeddingVector == null && !string.IsNullOrEmpty(message.Content))
                {
                    try
                    {
                        message.EmbeddingVector = await _embeddingService.GetEmbeddingAsync(message.Content);
        ...
        public async Task<List<Models.Chat>> FindSimilarChatsAsync(float[] embedding, int limit = 5)
        {
            List<Models.Chat> results;
            ...
            try
            {
                ...
                var allChats = new List<Models.Chat>();
                using (var command = _databaseConnection.CreateCommand())
                {
                    ...
                    if (_vectorSearchAvailable && segment.EmbeddingVector != null)
                    {
                        try
                        {
                            using (var vectorCommand = _databaseConnection.CreateCommand())
                            {
                                ...
                                {
                                    vectorCommand.CommandText =
                                        "DELETE FROM chat_segment_vectors WHERE segment_id = @segmentId";
