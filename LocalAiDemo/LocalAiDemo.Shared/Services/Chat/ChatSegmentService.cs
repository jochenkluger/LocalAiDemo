    ...
    public class ChatSegmentService : IChatSegmentService
    {
        ...
        public async Task<List<ChatSegment>> CreateDailySegmentsAsync(int chatId)
        {
            ...
            try
            {
                ...
                foreach (var dateGroup in messagesByDate)
                {
                    ...
                    {
                        segment.EmbeddingVector = await _embeddingService.GetEmbeddingAsync(segment.CombinedContent);
                    }
                    ...
                    _logger.LogDebug(
                        "Created and saved segment for {Date} with {MessageCount} messages, ID: {SegmentId}",
        ...
        public async Task<int> CreateSegmentsForAllChatsAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Completed segment creation. Created {TotalSegments} segments for {ChatCount} chats",
        ...
        public async Task<int> VectorizeUnprocessedSegmentsAsync()
        {
            ...
            try
            {
                ...
                foreach (var chat in allChats)
                {
                    ...
                    var unprocessedSegments = segments
                        .Where(s => s.EmbeddingVector == null && !string.IsNullOrEmpty(s.CombinedContent)).ToList();

                    foreach (var segment in unprocessedSegments)
                    {
                        try
                        {
                            segment.EmbeddingVector =
                                await _embeddingService.GetEmbeddingAsync(segment.CombinedContent);
                ...
                _logger.LogInformation("Completed segment vectorization. Processed {ProcessedCount} segments",
                    processedCount);
        ...
        public async Task<int> ReVectorizeAllSegmentsAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Completed segment re-vectorization. Processed {ProcessedCount} segments",
                    processedCount);
                return processedCount;
            }
            catch (Exception ex)
        ...
        public async Task<List<SegmentSimilarityResult>> FindSimilarSegmentsAsync(string query, int limit = 10)
        {
            ...
            try
            {
                ...
                foreach (var segment in allSegments)
                {
                    ...
                    {
                        var similarity =
                            _embeddingService.CalculateCosineSimilarity(queryEmbedding, segment.EmbeddingVector);
        ...
        public async Task<List<SegmentSimilarityResult>> FindSimilarSegmentsInChatAsync(int chatId, string query,
            int limit = 10)
        {
        ...
        public async Task<SegmentVectorizationStats> GetSegmentStatsAsync()
        {
            ...
            try
            {
                ...
                return new SegmentVectorizationStats
                {
                    ...
                    VectorizationPercentage = segmentsWithContent > 0
                        ? (double)segmentsWithVectors / segmentsWithContent * 100
                        : 0,
        ...
        public async Task UpdateSegmentsForChatAsync(int chatId)
        {
            ...
            try
            {
                ...
                if (todaysSegment != null)
                {
                    ...
                    _logger.LogDebug("Updating existing segment for {Date} in chat {ChatId}", today.ToShortDateString(),
                        chatId);
                    ...
                    {
                        todaysSegment.EmbeddingVector =
                            await _embeddingService.GetEmbeddingAsync(todaysSegment.CombinedContent);
                ...
                else
                {
                    ...
                    _logger.LogDebug("Creating new segment for {Date} in chat {ChatId}", today.ToShortDateString(),
                        chatId);
                    ...
                    {
                        newSegment.EmbeddingVector =
                            await _embeddingService.GetEmbeddingAsync(newSegment.CombinedContent);
                    }
        ...
        public async Task<List<ChatSegment>> CreateThematicSegmentsAsync(int chatId, float similarityThreshold = 0.7f)
        {
            ...
            try
            {
                ...
                {
                    messageEmbeddings[message] = await _embeddingService.GetEmbeddingAsync(message.Content);
                }
