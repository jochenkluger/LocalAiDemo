...
namespace LocalAiDemo.Shared.Services.Chat
{
    ...
    public class ContactService : IContactService
    {
        ...
        private async Task InitializeContactsFromDatabaseAsync()
        {
            ...
            try
            {
                ...
                var sampleContacts = new List<Contact>
                {
                    new Contact
                    {
                        Id = 1, Name = "Maria Schmidt", Email = "maria.schmidt@company.com",
                        Phone = GeneratePhoneNumber(), Status = ContactStatus.Online, Department = "Sales",
                        LastSeen = DateTime.Now, Avatar = GetAvatarForDepartment("Sales")
                    },
                    new Contact
                    {
                        Id = 2, Name = "Thomas Müller", Email = "thomas.mueller@company.com",
                        Phone = GeneratePhoneNumber(), Status = ContactStatus.Away, Department = "Engineering",
                        LastSeen = DateTime.Now.AddHours(-1), Avatar = GetAvatarForDepartment("Engineering")
                    },
                    new Contact
                    {
                        Id = 3, Name = "Julia Weber", Email = "julia.weber@company.com", Phone = GeneratePhoneNumber(),
                        Status = ContactStatus.Offline, Department = "Marketing", LastSeen = DateTime.Now.AddDays(-1),
                        Avatar = GetAvatarForDepartment("Marketing")
                    },
                    new Contact
                    {
                        Id = 4, Name = "Michael Wagner", Email = "michael.wagner@company.com",
                        Phone = GeneratePhoneNumber(), Status = ContactStatus.Online, Department = "Support",
                        LastSeen = DateTime.Now, Avatar = GetAvatarForDepartment("Support")
                    },
                    new Contact
                    {
                        Id = 5, Name = "Anna Fischer", Email = "anna.fischer@company.com",
                        Phone = GeneratePhoneNumber(), Status = ContactStatus.DoNotDisturb, Department = "Management",
                        LastSeen = DateTime.Now.AddMinutes(-30), Avatar = GetAvatarForDepartment("Management")
                    }
                };
        ...
        public async Task<Contact?> GetContactByNameAsync(string name)
        {
            ...
            var contact =
                _contacts.FirstOrDefault(c => string.Equals(c.Name, name, StringComparison.OrdinalIgnoreCase));
            return contact;
        }
        ...
        public async Task<Contact?> EnsureContactForPersonAsync(string personName)
        {
            ...
            // Neue Person in der Datenbank prüfen // Person dependency removed - returning null as
            // this functionality is deprecated
            _logger.LogWarning(
                "CreateContactFromPersonAsync called but Person functionality is removed. PersonName: {PersonName}",
                personName);
            return null;
        }
    }
