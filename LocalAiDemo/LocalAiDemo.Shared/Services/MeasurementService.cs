    ...
    public class MeasurementService : IMeasurementService, IDisposable
    {
        ...
        public MeasurementService(ILogger<MeasurementService>? logger = null)
        {
            ...
            _measurements = new List<Measurement>
            {
                ...
                new Measurement { Id = 2, Name = "Speichernutzung", Value = 3.7, Unit = "GB", Timestamp = DateTime.Now },
                new Measurement { Id = 3, Name = "Netzwerkverkehr", Value = 1.2, Unit = "MB/s", Timestamp = DateTime.Now },
            ...
            _logger?.LogInformation("MeasurementService initialized with {Count} measurements and timer started", _measurements.Count);
        ...
        private void UpdateMeasurements()
        {
            ...
            foreach (var measurement in _measurements)
            {
                ...
                {
                    case "CPU-Auslastung":
                        measurement.Value = Math.Min(100, Math.Max(5, measurement.Value + (_random.NextDouble() * 10 - 5)));
                        break;
                    ...
                    case "Netzwerkverkehr":
                        measurement.Value = Math.Min(5, Math.Max(0.1, measurement.Value + (_random.NextDouble() - 0.5)));
                        break;

                    case "Latenz":
                        measurement.Value = Math.Min(100, Math.Max(10, measurement.Value + (_random.NextDouble() * 10 - 5)));
                        break;

                    case "Temperatur":
                        measurement.Value = Math.Min(80, Math.Max(35, measurement.Value + (_random.NextDouble() * 2 - 1)));
                        break;
                }
