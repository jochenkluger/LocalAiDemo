    ...
    public class ChatVectorizationService : IChatVectorizationService
    {
        ...
        public async Task<int> VectorizeUnprocessedChatsAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Found {UnprocessedCount} chats without embeddings out of {TotalCount} total chats",
        ...
        public async Task<int> VectorizeUnprocessedMessagesAsync()
        {
            ...
            try
            {
                ...
                {
                    var unprocessedMessages = chat.Messages.Where(m => m.EmbeddingVector == null && !string.IsNullOrEmpty(m.Content)).ToList();
                ...
                _logger.LogInformation("Completed message vectorization. Processed {ProcessedCount} out of {TotalCount} messages",
        ...
        public async Task<int> ReVectorizeAllMessagesAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Completed message re-vectorization. Processed {ProcessedCount} messages", processedCount);
                return processedCount;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<ChatMessage>> FindSimilarMessagesAsync(string query, int limit = 10)
        {
            ...
            try
            {
                ...
                foreach (var message in allMessages)
                {
                    ...
                    {
                        var similarity = _embeddingService.CalculateCosineSimilarity(queryEmbedding, message.EmbeddingVector);
        ...
        public async Task<VectorizationStats> GetVectorizationStatsAsync()
        {
            ...
            try
            {
                ...
                var stats = new VectorizationStats
                {
                    ...
                    ChatVectorizationPercentage = allChats.Count > 0 ? (double)chatsWithVectors / allChats.Count * 100 : 0,
                    MessageVectorizationPercentage = nonEmptyMessages > 0 ? (double)messagesWithVectors / nonEmptyMessages * 100 : 0
                };

                _logger.LogDebug("Vectorization stats: {ChatsWithVectors}/{TotalChats} chats, {MessagesWithVectors}/{NonEmptyMessages} messages",
        ...
        public async Task UpdateMessageVectorAsync(int messageId)
        {
            ...
            try
            {
                ...
                targetMessage.EmbeddingVector = _embeddingService.GenerateEmbedding(targetMessage.Content);
