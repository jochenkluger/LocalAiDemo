    ...
    public class EmbeddingService : IEmbeddingService, IDisposable
    {
        ...
        public EmbeddingService(ILogger<EmbeddingService> logger)
        {
            ...
            try
            {
                ...
                var modelPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Models", "all-MiniLM",
                ...
                else
                {
                    ...
                    _logger.LogError(
                        "ONNX model not found at {ModelPath}. Please download the all-MiniLM-L6-v2 ONNX model from Hugging Face.",
                        modelPath);
        ...
        public async Task<float[]> GetEmbeddingAsync(string text)
        {
            ...
            if (_session == null)
            {
                throw new InvalidOperationException(
                    "ONNX model is not loaded. Please ensure the all-MiniLM-L6-v2 model files are available.");
            }
        ...
        private float[] GenerateOnnxEmbedding(string text)
        {
            try
            {
                ...
                _logger.LogDebug("Successfully generated {Dimension}-dimensional ONNX embedding",
